...
class precision_recall(object):
    ...
    def compute_precision_recall(self, dataloader, gen, d, num_generate, num_runs, num_clusters,
                                 batch_size, device, num_angles=1001):
        ...
        for i in tqdm(range(n_batches), disable = self.disable_tqdm):
            ...
            fake_images = self.generate_images(gen,  d, batch_size)
...
def calculate_f_beta_score(dataloader, gen, d, num_generate, num_runs, num_clusters, beta, device):
    inception_model = InceptionV3().to(device)
    inception_model.eval()

    batch_size = dataloader.batch_size
    PR = precision_recall(inception_model, device=device)
    precision, recall = PR.compute_precision_recall(dataloader, gen, d, num_generate, num_runs, num_clusters,  batch_size, device)
